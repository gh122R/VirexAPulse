...
/*
 * +-------------------------------------------------------------------------------------------------------------+
*  | Здесь происходить регистрация маршрутов. Чтобы зарегистрировать маршрут, обратитесь в Nexus и его методу    |
 * | Всего у Nexus есть 3 метода, доступные для вызова: get, post и view.                                        |
 ...
 * | Например, если вы зарегистрировали маршрут так: <<<Nexus::get('/', fn()=> 123)>>>, то если браузер          |
 * | отправит post http-запрос, вместо get, роутер выбросит исключение.                                          |
 * +-------------------------------------------------------------------------------------------------------------+
 * */

Nexus::get('/w', fn()=> 'welcome');
Nexus::get('/home/{id}', [HomeController::class, 'index']);
Nexus::view('/', 'welcome');
Nexus::get('/test', fn()=> 'test', fn($next) => $next());

/*
 * +-----------------------------------------------------------------------------------------------------------------------------------+
 * | Если вы регистрируете маршрут с помощью <<<Nexus::get>>> или <<<Nexus::post>>>, то                                                |
